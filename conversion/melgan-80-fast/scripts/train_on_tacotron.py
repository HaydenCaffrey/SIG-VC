...
from mel2wav.dataset import ParallelAudioDataset as AudioDataset
...
def parse_args():
    ...
    parser.add_argument("--seq_len", type=int, default=32*200)
...
def main():
    ...
    #(shiyao) : as are our current standard, the `archodd` edition of this project
    # are meant to fit this particular hyper-parameter setup
    fft = Audio2Mel(
        n_fft = 800,
        hop_length = 200,
        win_length = 800,
        sampling_rate = 16000,
        n_mel_channels = args.n_mel_channels,   # (80)
        mel_fmin = 55.0,
        mel_fmax = 7600.0
    ).cuda()
    ...
    train_set = AudioDataset(
        Path(args.data_path) / "train_files.txt", args.seq_len, sampling_rate=args.samplerate, use_numpy_audio=True
    )
    ...
    test_voc = []
    test_audio = []
    for i, batch_t in enumerate(test_loader):
        x_t, s_t = batch_t
        x_t = x_t.cuda()
        s_t = s_t.cuda()        #s_t = fft(x_t).detach()

        test_voc.append(s_t.cuda())
        test_audio.append(x_t)

        audio = x_t.squeeze().cpu()
        save_sample(root / ("original_%d.wav" % i), args.samplerate, audio)
        writer.add_audio("original/sample_%d.wav" % i, audio, 0, sample_rate=args.samplerate)
    ...
    for epoch in range(1, args.epochs + 1):
        for iterno, batch_t in enumerate(train_loader):
            x_t, s_t = batch_t

            x_t = x_t.cuda()                              # shape(bcsz, 1, time_x)
            s_t = s_t.cuda()  # s_t = fft(x_t).detach()     shape(bcsz, mel, time_s)
            x_pred_t = netG(s_t.cuda())                 #   shape: same as x_t
            ...
            for i in range(args.num_D):
                for j in range(len(D_fake[i]) - 1):
                    # print(f'd_fake[{i}][{j}] : {D_fake[i][j].size()}, d_real[{i}][{j}] : {D_real[i][j].size()}')
            ...
            if steps % args.save_interval == 0:
                ...
                with torch.no_grad():
                    for i, (voc, _) in enumerate(zip(test_voc, test_audio)):
                        ...
                        save_sample(root / ("generated_%d.wav" % i), args.samplerate, pred_audio)
                        writer.add_audio(
