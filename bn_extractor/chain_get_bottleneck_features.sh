...
frame_subsampling_factor=$(cat $nnetdir/frame_subsampling_factor)
...
cp $nnetdir/frame_subsampling_factor $bnfdir/
...
if [ $stage -le 1 ]; then
  ...
  $cmd $compute_queue_opt JOB=1:$nj $logdir/make_bnf_$name.JOB.log \
    nnet3-compute $compute_gpu_opt $ivector_opts \
      --frame-subsampling-factor=$frame_subsampling_factor "$modified_bnf_nnet" "$feats" ark:- \| \
